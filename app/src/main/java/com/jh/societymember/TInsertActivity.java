...
//                String strEmail=bundle.getString("strEmail");
//                String strPwd=bundle.getString("strPwd");
//                //파이어 베이스에 들어가는 아이디 비번 가져오는 것 부터
                ...
                String leader=et_leader.getText().toString();
                    ...
                    // 현재 토큰으로 생성된 유저 데이터 베이스에 추가 하는 행위!
                    TInsertAccount Taccount = new TInsertAccount();
                    Taccount.setTeamName(teamname);
                    Taccount.setRegion(region);
                    Taccount.setSports(Sports);
                    Taccount.setLeader(leader);

                    mDatabaseRef.child("TInsertAccount").child(firebaseUser.getUid()).setValue(Taccount);
                    ...
                    // -> 기존의 회원가입과는 달리 파이어베이스 유저를 생성할 필요가 없다 ! 미리 생성했기 때문!
